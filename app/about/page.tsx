...
function CtaButton({ children, href = applyHref, variant = "warning" }: { children: React.ReactNode; href?: string; variant?: "warning" | "primary" | "secondary" }) {
  ...
  if (variant === "secondary") {
    return <Link href={href} {...analyticsProps} className={buttonClass("secondary", "lg", "w-full rounded-full px-7 text-center font-bold !text-black hover:!text-black sm:w-auto")}>{children}</Link>;
  }
