...
const values = [
  ["사건 유형별 교육", "음주운전, 폭력범죄, 도박중독, 성범죄 등 사건 유형에 맞는 재범방지교육을 제공합니다."],
...
export default function AboutPage() {
  return (
    <main className="keep-korean min-h-screen bg-slate-50 text-slate-950">
      ...
      <section className="px-4 py-16 sm:px-6 lg:px-8"><div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr]"><SectionTitle eyebrow="About" title="행동 변화를 위한 온라인 재범방지교육" body="교육은 사건 유형별 위험 요인과 행동 패턴을 이해하고, 실제 생활에서 적용할 수 있는 대처 행동과 생활 관리 방법을 익히는 데 중점을 둡니다. 교육 수강 후에는 과정에 맞는 수료증 또는 이수증을 확인하고 출력할 수 있습니다." /><div className="grid gap-4 sm:grid-cols-2">{values.map(([title, body]) => <article key={title} className="rounded-[1rem] border border-slate-200 bg-white p-5 shadow-sm"><Icon className="h-6 w-6 text-[#176b68]" /><h3 className="mt-4 text-lg font-black">{title}</h3><p className="mt-3 text-sm leading-7 text-slate-700">{body}</p></article>)}</div></div></section>

      <section className="bg-white px-4 py-16 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><SectionTitle eyebrow="Courses" title="운영 교육과정" body="음주운전 수료증과 양형자료 준비를 중심으로 운영해 온 구조를 유지하면서, 폭력범죄·도박중독·성범죄·마약중독·디지털범죄 재범방지교육까지 확장했습니다." /><div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{platformCourseCategories.map((course) => <Link key={course.slug} href={"/courses/" + course.slug} className="rounded-[1rem] border border-slate-200 bg-slate-50 p-5 transition hover:-translate-y-0.5 hover:border-[#176b68] hover:bg-white hover:shadow-[0_16px_36px_rgba(15,23,42,0.1)]"><h3 className="text-lg font-black text-slate-950">{course.title}</h3><p className="mt-3 text-sm leading-7 text-slate-700">{course.description}</p></Link>)}</div></div></section>
