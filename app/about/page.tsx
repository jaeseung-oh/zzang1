...
export default function AboutPage() {
  return (
    <main className="keep-korean min-h-screen bg-slate-50 text-slate-950">
      ...
      <section className="bg-white px-4 py-16 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><SectionTitle eyebrow="Courses" title="운영 교육과정" body="음주운전 수료증과 양형자료 준비를 중심으로 운영해 온 구조를 유지하면서, 폭력범죄·도박중독·성범죄 재범방지교육까지 확장했습니다." /><div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{platformCourseCategories.map((course) => <Link key={course.slug} href={"/courses/" + course.slug} className="rounded-[1rem] border border-slate-200 bg-slate-50 p-5 transition hover:-translate-y-0.5 hover:border-[#176b68] hover:bg-white hover:shadow-[0_16px_36px_rgba(15,23,42,0.1)]"><h3 className="text-lg font-black text-slate-950">{course.title}</h3><p className="mt-3 text-sm leading-7 text-slate-700">{course.description}</p></Link>)}</div></div></section>
