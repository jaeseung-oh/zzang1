...
export default function HomePage() {
  ...
  return (
    <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(organizationJsonLd).replace(/</g, "\\u003c") }} /><main className="keep-korean min-h-screen bg-slate-50 text-slate-950">
      ...
      <section className="bg-white px-4 py-16 sm:px-6 lg:px-8"><div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.9fr_1.1fr]"><SectionTitle title="민간 온라인 예방교육 플랫폼입니다" body="ResetEdu 재발방지교육센터는 교육 이수와 실천자료 정리를 돕는 온라인 플랫폼입니다. 변호사 사무실이나 공공기관이 아니며, 법률대리·법률자문·결과 보장을 제공하지 않습니다." /><div className="rounded-[1rem] border border-amber-200 bg-amber-50 p-6 text-sm leading-7 text-amber-950">수료증, 이수증 및 실천자료는 교육 참여와 재발방지 노력을 정리하기 위한 민간 교육자료입니다. 제출 여부와 활용 가능성은 개별 사건과 제출기관의 판단에 따라 달라질 수 있습니다.</div></div></section>
