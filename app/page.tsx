...
const materialsButtonClass = "inline-flex min-h-14 w-full items-center justify-center rounded-xl border-2 border-amber-300 bg-white px-7 py-4 text-center text-base font-black text-slate-950 shadow-[0_16px_34px_rgba(250,204,21,0.24)] transition hover:-translate-y-0.5 hover:bg-amber-50 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-200 sm:w-auto sm:rounded-full";
      ...
      <section className="bg-white px-4 py-20 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl"><SectionTitle eyebrow="Start Here" title="양형자료 준비는 교육 수료증과 재발방지 자료부터 시작하세요" body="음주운전 사건 이후 반성과 개선 노력을 말로만 설명하기보다, 교육 이수 기록과 재발방지 실천계획으로 정리하세요. Reset Edu Center는 수강, 수료증 발급, 자료 작성과 출력까지 바로 이어지는 구조로 제공합니다." />
          <div className="mt-10 grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
            {categories.map(([title, status, body]) => <article key={title} className="rounded-xl border-2 border-indigo-700 bg-indigo-50 p-6"><div className="flex items-center justify-between gap-3"><Icon name="car" className="h-8 w-8 text-indigo-700" /><span className="rounded-full bg-indigo-600 px-3 py-1 text-xs font-black text-white">{status}</span></div><h3 className="mt-5 text-2xl font-black leading-snug text-slate-950">{title}</h3><p className="mt-3 text-base leading-8 text-slate-700">{body}</p><div className="mt-6 flex flex-wrap gap-3"><Link href={applyHref} data-ga-event="click_enroll" data-ga-item-id="dui-documents" data-ga-item-name="음주운전 예방교육" data-ga-location="home" className={buttonClass("primary", "md", "rounded-xl font-black")}>지금 수강 신청하기</Link><Link href="#materials" className="inline-flex min-h-12 items-center justify-center rounded-xl border-2 border-amber-300 bg-white px-5 py-3 text-sm font-black text-slate-950 shadow-[0_12px_24px_rgba(250,204,21,0.18)] transition hover:bg-amber-50 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-amber-200">제공 자료 확인하기</Link></div></article>)}
