  ...
  return (
    <><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(organizationJsonLd).replace(/</g, "\u003c") }} /><main className="keep-korean min-h-screen bg-slate-50 text-slate-950">
      <section className="relative overflow-hidden bg-[#eef4f8]">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 sm:py-14 lg:grid-cols-[minmax(0,0.92fr)_minmax(480px,1.08fr)] lg:px-8 lg:py-20">
          <div className="flex min-w-0 flex-col justify-center">
            <p className="text-sm font-black leading-6 text-[#173968]">사건 이후의 반성과 재발방지 노력을 체계적인 양형자료로 준비하세요</p>
            <h1 className="mt-4 max-w-3xl text-[2rem] font-black leading-tight text-slate-950 sm:mt-5 sm:text-5xl lg:text-[52px]">온라인 예방교육부터<br /><span className="sm:whitespace-nowrap">양형자료 준비까지 한 번에</span></h1>
            <p className="mt-5 max-w-2xl text-[15px] leading-7 text-slate-700 sm:mt-6 sm:text-lg sm:leading-8">음주운전 사건 이후 필요한 예방교육을 온라인으로 수강하고, 교육 이수증과 재발방지 실천자료를 통해 반성과 변화의 노력을 체계적으로 정리할 수 있습니다.</p>
            <div className="mt-7 grid gap-3 sm:mt-8 sm:flex sm:flex-wrap">
              <Link href="#pricing" className={buttonClass("primary", "lg", "w-full whitespace-nowrap rounded-full px-7 font-black sm:w-auto")}>교육과정 확인하기</Link>
          ...
          <div className="grid gap-3 sm:hidden">
            <div className="grid grid-cols-[0.9fr_1.1fr] gap-3">
              <div className="rounded-[1.15rem] border-[6px] border-slate-950 bg-white shadow-[0_18px_38px_rgba(15,23,42,0.18)]">
                <div className="rounded-[0.75rem] bg-white p-3">
              ...
              <div className="grid gap-3">
                <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm">
                  <p className="text-sm font-black text-emerald-900">수강 즉시 시작</p>
                  <p className="mt-1 text-xs font-bold leading-5 text-emerald-800">모바일에서도 강의실과 출력 자료를 확인할 수 있습니다.</p>
                </div>
                <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
                  <p className="text-xs font-black text-slate-500">제공 자료</p>
                  <p className="mt-2 text-sm font-black leading-5 text-slate-900">수료증 · 재발방지계획서 · 서약서</p>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="overflow-hidden rounded-xl border border-[#d6bd7d] bg-white shadow-sm">
          ...
          <div className="relative hidden min-h-[520px] sm:block lg:min-h-[610px]">
