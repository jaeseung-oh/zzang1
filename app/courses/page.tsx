...
export const metadata: Metadata = {
  title: "강의 구성 | 리셋 에듀센터",
  description: "현재 운영 중인 음주운전 예방교육과 준비중인 교육 과정을 확인하는 페이지",
};
...
export default function CoursesPage() {
  return (
    <main className="keep-korean min-h-screen bg-slate-50 text-slate-950">
      ...
      <section className="border-b border-slate-200 bg-white px-4 py-5 sm:px-6 lg:px-8"><div className="mx-auto flex max-w-7xl gap-3 overflow-x-auto pb-1">{tabs.map(([title, status, active]) => <div key={title} className={active ? "min-w-[210px] rounded-2xl border-2 border-indigo-600 bg-indigo-50 p-4 text-slate-950" : "min-w-[210px] rounded-2xl border border-slate-200 bg-slate-100 p-4 text-slate-700 opacity-90"}><div className="flex items-center justify-between gap-3"><p className="text-sm font-black leading-snug">{title}</p><span className={active ? "rounded-full bg-indigo-600 px-2.5 py-1 text-xs font-black text-white" : "rounded-full bg-gray-300 px-2.5 py-1 text-xs font-black text-gray-700"}>{status}</span></div></div>)}</div></section>
      ...
      <section className="bg-white px-4 py-14 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><SectionTitle title="상품 구성" body="수강 즉시 수료증 출력 과정과 참고서식 포함 과정으로 구성되어 있습니다. 필요한 자료 구성에 따라 선택할 수 있습니다." /><div className="mt-8 grid gap-5 lg:grid-cols-2"><ProductCard product={basicApplicationProduct} /><ProductCard product={duiDocumentsApplicationProduct} /></div><div className="mt-8 flex flex-wrap gap-3"><Link href={applyHref} className={buttonClass("primary", "lg", "whitespace-nowrap rounded-full px-7 font-black")}>음주운전 교육 수강 신청</Link><Link href="/courses/apply" className={buttonClass("secondary", "lg", "whitespace-nowrap rounded-full px-7 font-bold")}>수강신청 이동</Link></div></div></section>
