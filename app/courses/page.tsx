...
import { formatApplicationKrw, basicApplicationProduct, duiDocumentsApplicationProduct, duiCbtAdvancedApplicationProduct } from "@/lib/course/application-products";

export const metadata: Metadata = {
  title: "온라인 재발방지교육·교육 수료증 과정 | Reset Edu Center",
  description: "음주운전 예방교육의 교육내용과 수료증, 재발방지 실천자료 제공 여부를 확인해 보세요.",
...
export default function CoursesPage() {
  return (
    <main className="keep-korean min-h-screen bg-slate-50 text-slate-950">
      ...
      <section className="bg-white px-4 py-14 sm:px-6 lg:px-8"><div className="mx-auto max-w-7xl"><SectionTitle title="필요한 준비 범위에 따라 선택하세요" body="59,000원 기본과정, 109,000원 서식 포함 과정, 290,000원 인지행동기반 재발방지교육 심화과정을 비교한 후 선택할 수 있습니다." /><div className="mt-8 grid gap-5 lg:grid-cols-3"><ProductCard product={basicApplicationProduct} /><ProductCard product={duiDocumentsApplicationProduct} /><ProductCard product={duiCbtAdvancedApplicationProduct} /></div><div className="mt-8 flex flex-wrap gap-3"><Link href={applyHref} className={buttonClass("primary", "lg", "whitespace-nowrap rounded-full px-7 font-black !text-white hover:!text-white")}>기본과정 59,000원</Link><Link href="/courses/apply?category=dui&productId=dui-documents" className={buttonClass("primary", "lg", "whitespace-nowrap rounded-full px-7 font-black !text-white hover:!text-white")}>교육·자료 포함 과정 109,000원</Link><Link href="/courses/apply?category=dui&productId=dui-cbt-advanced" className={buttonClass("primary", "lg", "whitespace-nowrap rounded-full px-7 font-black !text-white hover:!text-white")}>심화과정 290,000원</Link></div></div></section>
