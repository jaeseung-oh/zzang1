      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-2 xl:grid-cols-3">
          {products.map((product) => {
            ...
            return (
              <article key={product.id} className={isAdvanced ? "flex min-h-full flex-col rounded-[1.25rem] border-2 border-[#173968] bg-white p-6 shadow-[0_18px_45px_rgba(23,57,104,0.12)]" : "flex min-h-full flex-col rounded-[1.25rem] border border-slate-200 bg-slate-50 p-6 shadow-sm"}>
                ...
                <Link href={getApplyHref(course, product.id)} className={buttonClass(isAdvanced ? "primary" : "warning", "md", isAdvanced ? "mt-6 w-full rounded-xl font-black" : "mt-6 w-full rounded-xl font-black !text-black hover:!text-black")}>{product.title} 신청하기</Link>
              </article>
            );
          })}
        </div>
