...
export function CourseAccessGuard({ courseId, children }: { courseId: string; children: React.ReactNode }) {
  ...
  if (state === "denied") return <AccessDeniedCard message="이 강의는 결제 완료 후 수강할 수 있습니다." primaryHref="/courses/apply?categoryId=dui" primaryLabel="수강 신청하기" secondaryHref="/courses/dui-prevention" secondaryLabel="강의 구성 보기" />;
  if (state === "error") return <AccessDeniedCard message="수강권 확인 중 문제가 발생했습니다. 잠시 후 다시 시도해 주세요." primaryHref="/courses/apply?categoryId=dui" primaryLabel="수강 신청하기" secondaryHref="/" secondaryLabel="홈으로 이동" />;
