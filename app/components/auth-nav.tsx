  ...
  if (!state.user) {
    return (
      <div className="flex min-w-0 flex-1 flex-wrap items-center justify-end gap-2 sm:flex-none sm:flex-nowrap">
        ...
        <Link href="/signup" className={buttonClass("primary", "sm", "whitespace-nowrap rounded-full px-4 font-bold")}>회원가입</Link>
        <Link href={applyHref} className={buttonClass("warning", "sm", "whitespace-nowrap rounded-full px-4 font-black")}>수강 신청하기</Link>
      </div>
    );
  }
