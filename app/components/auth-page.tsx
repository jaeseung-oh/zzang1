...
async function sendVerificationEmail(user: User) {
  try {
    await sendEmailVerification(user, {
      url: `${getAppOrigin()}/login?verified=true`,
...
function getAuthErrorMessage(error: unknown, fallback = "요청 처리 중 문제가 발생했습니다. 잠시 후 다시 시도해 주세요.") {
  ...
  switch (code) {
    ...
    case "auth/unauthorized-continue-uri":
      return "인증 완료 이동 주소가 Firebase 승인 도메인에 등록되어 있지 않습니다. 관리자에게 resetedu.kr 승인 도메인 설정을 요청해 주세요.";
    case "auth/invalid-continue-uri":
      return "인증 완료 이동 주소 형식이 올바르지 않습니다. 관리자에게 인증메일 URL 설정을 요청해 주세요.";
    case "auth/missing-continue-uri":
      return "인증 완료 이동 주소가 누락되었습니다. 관리자에게 인증메일 URL 설정을 요청해 주세요.";
    ...
    startSubmitTransition(async () => {
      try {
        ...
        try {
          ...
          await recordVerificationEmailAttempt(credential.user.uid, { ok: true });
          setVerificationCooldown(60);
        } catch (verificationError) {
          verificationSent = false;
          console.error(verificationError);
          await recordVerificationEmailAttempt(credential.user.uid, { ok: false, errorCode: getAuthErrorCode(verificationError), errorMessage: verificationError instanceof Error ? verificationError.message : null }).catch(() => undefined);
        ...
        setMessage(
          verificationSent
            ? "회원가입이 완료되었고 인증 메일을 발송했습니다. 메일함과 스팸함을 확인해 주세요."
            : "회원가입은 완료되었지만 인증 메일 발송에는 실패했습니다. 아래 오류를 확인한 뒤 재발송을 시도해 주세요."
        );
      } catch (submitError) {
  ...
  const handleResendVerification = () => {
    ...
    void (async () => {
      try {
        ...
        await recordVerificationEmailAttempt(user.uid, { ok: true });
        setMessage("인증 메일을 다시 보냈습니다. 메일함과 스팸함을 확인해 주세요.");
        setVerificationCooldown(60);
      } catch (verificationError) {
        console.error(verificationError);
        if (resendUser) {
          await recordVerificationEmailAttempt(resendUser.uid, { ok: false, errorCode: getAuthErrorCode(verificationError), errorMessage: verificationError instanceof Error ? verificationError.message : null }).catch(() => undefined);
        }
        setError(getAuthErrorMessage(verificationError));
        if (isAuthError(verificationError, "auth/too-many-requests")) {
          setVerificationCooldown(300);
