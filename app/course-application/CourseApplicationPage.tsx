        ...
        <div className="mt-7 grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="space-y-6">
            ...
            <section className="rounded-[1.5rem] border border-[#dbe4ef] bg-white p-5 shadow-[0_18px_45px_rgba(15,23,42,0.07)] sm:p-6">
              ...
              {isAvailable ? <div className="mt-5 grid gap-4 xl:grid-cols-2">{selectedCategory.products.map((product) => { const isSelected = selectedProduct?.id === product.id; return <button key={product.id} type="button" aria-selected={isSelected} onClick={() => setSelectedProductId(product.id)} className={(isSelected ? selectedChoiceClass : selectableCardClass) + " p-5"}><div className="flex items-start justify-between gap-4"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className={isSelected ? "text-lg font-black leading-snug text-white" : "text-lg font-black leading-snug text-slate-950"}>{product.title}</h3>{product.badge ? <span className={isSelected ? "rounded-full border border-white/25 bg-white/12 px-2.5 py-1 text-xs font-black text-white" : "rounded-full bg-[#eef4fb] px-2.5 py-1 text-xs font-black text-[#173968]"}>{product.badge}</span> : null}</div><p className={isSelected ? "mt-2 text-sm leading-relaxed text-slate-100" : "mt-2 text-sm leading-relaxed text-slate-700"}>{product.description}</p></div><span className={isSelected ? "flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-white text-[#173968]" : "h-7 w-7 shrink-0 rounded-full border border-[#cbd5e1] bg-white"}>{isSelected ? <CheckIcon /> : null}</span></div><p className={isSelected ? "mt-5 text-3xl font-black text-white" : "mt-5 text-3xl font-black text-slate-950"}>{formatApplicationKrw(product.price)}</p><p className={isSelected ? "mt-1 text-xs font-bold text-[#f7d9a0]" : "mt-1 text-xs font-bold text-slate-500"}>{product.id === "dui-documents" ? "3종 참고서식 출력/PDF 포함" : "강의 수강 및 수강 즉시 수료증 출력"}</p><ul className="mt-5 space-y-2.5">{product.includes.map((item) => <li key={item} className={isSelected ? "flex min-w-0 gap-2 text-sm leading-relaxed text-slate-100" : "flex min-w-0 gap-2 text-sm leading-relaxed text-slate-800"}><span className={isSelected ? "mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-white text-[#173968]" : "mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#e7f0ff] text-[#173968]"}><CheckIcon className="h-3.5 w-3.5" /></span><span className="min-w-0">{item}</span></li>)}</ul></button>; })}</div> : <div className="mt-5 rounded-[1.25rem] border border-slate-200 bg-slate-100 p-5"><h3 className="text-lg font-black text-slate-950">준비중인 과정입니다.</h3><p className="mt-3 text-sm leading-relaxed text-slate-700">{selectedCategory.comingSoonText || "해당 교육 과정은 현재 콘텐츠를 준비하고 있습니다."}</p><p className="mt-2 text-sm leading-relaxed text-slate-700">과정 구성과 제공 자료가 확정되면 순차적으로 안내될 예정입니다.</p><button type="button" disabled className={buttonClass("secondary", "sm", "mt-5 rounded-xl px-5 font-bold disabled:bg-gray-300 disabled:text-gray-800")}>준비중</button></div>}
            </section>
          </div>
