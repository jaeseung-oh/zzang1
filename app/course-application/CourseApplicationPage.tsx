...
export default function CourseApplicationPage() {
  ...
  return (
    <main className="keep-korean min-h-screen bg-[linear-gradient(180deg,#f8fafc_0%,#edf3f9_48%,#f8fafc_100%)] px-4 py-8 text-slate-950 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <section className="overflow-hidden rounded-[1.75rem] border border-[#d7e1ef] bg-[linear-gradient(135deg,#0b1d36_0%,#173968_58%,#21568f_100%)] px-6 py-8 text-white shadow-[0_26px_70px_rgba(15,23,42,0.22)] sm:px-8 lg:px-10">
          ...
          <div className="mt-4 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between"><div className="min-w-0"><h1 className="text-3xl font-black leading-tight text-white sm:text-4xl">수강신청</h1><p className="mt-4 max-w-3xl text-sm leading-relaxed text-slate-100 sm:text-base">현재 실제 신청 가능한 과정은 음주운전 예방교육입니다. 준비중 과정은 선택해도 결제를 진행할 수 없습니다.</p></div><div className="flex flex-wrap gap-2 text-sm font-bold text-slate-100">{["온라인 수강", "수강 즉시 수료증 출력", "참고서식 선택"].map((badge) => <span key={badge} className="rounded-full border border-white/20 bg-white/10 px-4 py-2">{badge}</span>)}</div></div>
        </section>
            ...
            <section className="rounded-[1.5rem] border border-[#dbe4ef] bg-white p-5 shadow-[0_18px_45px_rgba(15,23,42,0.07)] sm:p-6">
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {applicationCourseCategories.map((category) => {
                  ...
                  return <button key={category.id} type="button" aria-selected={isSelected} onClick={() => handleCategorySelect(category)} className={(isSelected ? selectedCardClass : selectableCardClass) + " group min-h-[210px] p-4 " + (!available ? "bg-slate-100 text-slate-700" : "") }><div className="flex items-start justify-between gap-3"><span className={isSelected ? "flex h-11 w-11 items-center justify-center rounded-2xl bg-[#173968] text-white" : "flex h-11 w-11 items-center justify-center rounded-2xl bg-[#eef4fb] text-[#173968] transition group-hover:bg-[#dbeafe]"}><CourseIcon name={category.icon} /></span><StatusBadge available={available} /></div><h3 className="mt-4 text-lg font-black leading-snug text-slate-950">{category.title}</h3><p className="mt-2 min-h-[54px] text-sm leading-relaxed text-slate-700">{category.description}</p><p className="mt-3 text-xs font-bold text-slate-700">{category.summary}</p><div className="mt-4 flex items-end justify-between gap-3">{available ? <p className="text-sm font-black text-indigo-800">신청 가능</p> : <p className="text-sm font-black text-gray-700">결제 불가</p>}<span className={isSelected ? "rounded-full bg-[#173968] px-3 py-1 text-xs font-bold text-white" : "rounded-full bg-slate-200 px-3 py-1 text-xs font-bold text-slate-700"}>{isSelected ? "선택됨" : "확인"}</span></div></button>;
                })}
              </div>
            </section>
