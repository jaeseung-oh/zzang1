...
function ManualEnrollmentGrantPanel({ onRefresh }: { onRefresh: () => void }) {
  ...
  return <section className="mb-4 rounded-[1.25rem] border border-blue-200 bg-blue-50 p-4 text-sm text-blue-950"><p className="font-bold">수강권 직접 부여</p><p className="mt-1 text-xs leading-5">결제번호가 없거나 긴급 복구가 필요한 경우 관리자 권한으로 특정 회원 UID에 수강권을 직접 지급합니다. 이미 활성 수강권이 있으면 중복 지급하지 않습니다.</p><div className="mt-3 grid gap-2 md:grid-cols-[1fr_220px_130px_1.4fr_auto]"><input value={uid} onChange={(e) => setUid(e.target.value)} placeholder="사용자 ID(uid)" className="min-h-11 rounded-xl border border-blue-200 bg-white px-3 outline-none focus:border-blue-500" /><select value={productId} onChange={(e) => { setProductId(e.target.value); setAmount(getAdminGrantProductAmount(e.target.value)); }} className="min-h-11 rounded-xl border border-blue-200 bg-white px-3 outline-none focus:border-blue-500">{renderAdminGrantProductOptions()}</select><input value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="금액" className="min-h-11 rounded-xl border border-blue-200 bg-white px-3 outline-none focus:border-blue-500" /><input value={note} onChange={(e) => setNote(e.target.value)} placeholder="지급 사유" className="min-h-11 rounded-xl border border-blue-200 bg-white px-3 outline-none focus:border-blue-500" /><button type="button" onClick={handleGrant} disabled={submitting} className="rounded-xl bg-[#173968] px-4 py-2 font-bold text-white disabled:bg-gray-300">{submitting ? "처리 중" : "수동 지급"}</button></div>{status ? <p className="mt-3 font-semibold">{status}</p> : null}</section>;
}
