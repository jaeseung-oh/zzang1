...
NEXT_PUBLIC_PAYMENT_PROVIDER=portone-kcp-v2
NEXT_PUBLIC_PORTONE_STORE_ID=store-...
NEXT_PUBLIC_PORTONE_KCP_CHANNEL_KEY=channel-key-...
...
```bash
npx wrangler secret put PORTONE_API_SECRET --config wrangler.worker.toml
...
2. PortOne Browser SDK로 NHN KCP 결제창 호출
3. 성공 시 `/payment/success?paymentId=...&courseId=...&categoryId=...&productId=...` 이동
...
7. Worker가 PortOne API로 NHN KCP 결제내역을 서버 측 검증
...
- `paymentKeys/{paymentId}`: PortOne paymentId 중복 승인 방지 마커
- `refundPolicies/{courseId}`: 심사용 환불정책 설정
- `paymentLogs/{orderId_timestamp}`: 승인 또는 DB 저장 실패 재처리 로그

## 6. 운영 전 결제 점검표

- PortOne 콘솔: NHN KCP 실연동 채널과 결제수단 설정 확인
- 결제사 계약/심사: 카카오페이 계약 및 승인 확인
- Cloudflare Pages: `NEXT_PUBLIC_PORTONE_STORE_ID`, `NEXT_PUBLIC_PORTONE_KCP_CHANNEL_KEY` 운영 값 설정
- Cloudflare Worker: `PORTONE_API_SECRET` 운영 Secret Key 설정
